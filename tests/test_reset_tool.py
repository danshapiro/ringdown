...
class TestResetTool:
    ...
    def test_reset_tool_response_structure(self):
        ...
        assert set(result.keys()) == expected_keys, (
            f"Reset tool should return exactly these keys: {expected_keys}"
        )
    ...
    async def test_reset_tool_yields_reset_marker(self):
                            ...
                            "tool_calls": [
                                {
                                    "index": 0,
                                    "id": "call_123",
                                    "function": {"name": "reset", "arguments": '{"confirm": true}'},
                                }
                            ]
                        },
                        finish_reason=None,
        ...
        reset_markers = [
            t
            for t in collected_output
            if isinstance(t, dict) and t.get("type") == "reset_conversation"
        ]
        ...
        assert "resetting" not in text_tokens, (
            "Reset message should not be yielded as text - WebSocket handler sends it"
        )
