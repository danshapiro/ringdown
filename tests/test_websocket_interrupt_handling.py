    ...
    with patch.object(websocket_api, "is_from_twilio", return_value=True):
        ...
        messages = [
            ...
            (
                ...
                '"The latest session had your party", '
        ...
        # Mock dependencies
        with (
            patch.object(websocket_api, "get_agent_config") as mock_get_agent,
            patch.object(websocket_api, "pop_call", return_value=None),
            patch.object(websocket_api, "stream_response") as mock_stream,
            patch.object(websocket_api, "log_turn"),
            patch.object(
                websocket_api,
                "run_in_threadpool",
                side_effect=lambda f, *args: None,
            ),
        ):
...
async def test_malformed_json_handling():
    ...
    with patch.object(websocket_api, "is_from_twilio", return_value=True):
        ...
        error_calls = [
            call for call in mock_ws.send_json.call_args_list if "malformed message" in str(call)
        ]
...
def test_interrupt_message_structure():
    ...
    interrupt_msg = {
        ...
        "utteranceUntilInterrupt": (
            " The latest session had your party defeating an Elder Brain "
            "in the mind flayer stronghold Illithinoch, "
        ),
        "durationUntilInterruptMs": 938,
    }
    ...
    assert (
        interrupt_msg.get("utteranceUntilInterrupt")
        == (
            " The latest session had your party defeating an Elder Brain "
            "in the mind flayer stronghold Illithinoch, "
        )
...
async def test_interrupt_does_not_break_conversation_flow():
    ...
    with patch.object(websocket_api, "is_from_twilio", return_value=True):
        ...
        messages = [
            ...
            (
                '{"type": "interrupt", "utteranceUntilInterrupt": '
                '"Once upon a time", "durationUntilInterruptMs": 500}'
            ),
