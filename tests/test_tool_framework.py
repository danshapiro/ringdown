        ...
        for tool in tools_that_should_not_be_in_prompt:
            ...
            if expected_indicator:
                assert expected_indicator not in prompt, (
                    f"{agent_name} has unexpected prompt section for excluded "
                    f"tool {tool}: {expected_indicator}"
                )
...
def test_tool_response_truncation():
    ...
    large_dict = {"data": "x" * 250000, "status": "success", "metadata": {"type": "test"}}
    ...
    for provider in target_providers:
        ...
        if not provider_result["compatible"]:
            ...
            results["issues"].extend(
                [f"{provider}: {issue}" for issue in provider_result["issues"]]
            )
...
def _validate_provider_compatibility(
    ...
    if provider == "anthropic":
        ...
        if "$schema" not in params_schema:
            ...
            issues.append(
                f"Incorrect $schema version: {params_schema['$schema']} (should include 2020-12)"
            )
